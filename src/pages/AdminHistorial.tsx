      ...
      <div className="p-8 w-full min-h-screen bg-slate-50">
        ...
        <div className="bg-white rounded-2xl border border-slate-200 shadow-sm p-8 mb-8">
            <div className="flex items-center gap-4 mb-2">
                ...
                <div>
                    <h1 className="text-3xl font-bold text-slate-900">{clienteSeleccionado.fullname}</h1>
        ...
        <h3 className="text-xl font-bold text-slate-800 mb-4">Historial de Citas</h3>
                    ...
                    {clienteSeleccionado.historial.map(t => (
                        <tr key={t.id} className="hover:bg-slate-50">
                            <td className="px-6 py-4 text-slate-700 font-bold">
                                {new Date(t.fecha).toLocaleDateString('es-AR')} <span className="text-slate-400 font-normal">{new Date(t.fecha).toLocaleTimeString('es-AR', {hour:'2-digit', minute:'2-digit'})}</span>
                            </td>
                            <td className="px-6 py-4 text-slate-600">{t.servicio?.nombre}</td>
                            <td className="px-6 py-4 text-slate-600">{t.barbero?.fullname}</td>
    ...
    <div className="p-8 w-full min-h-screen bg-slate-50">
      ...
      <div className="flex flex-col md:flex-row justify-between items-end md:items-center mb-8 gap-4">
        <div>
            ...
            <p className="text-slate-500 mt-1">Directorio de personas que han visitado tu barbería.</p>
        </div>
        <div className="relative w-full md:w-72">
            <Search className="absolute left-3 top-1/2 -translate-y-1/2 text-slate-400" size={18} />
            <input 
                type="text" placeholder="Buscar cliente..." 
                className="w-full pl-10 pr-4 py-2.5 bg-white border border-slate-200 rounded-xl text-sm focus:outline-none focus:border-blue-500 shadow-sm transition"
            ...
            <tbody className="divide-y divide-slate-100">
                {clientesFiltrados.length === 0 && !loading ? (
                    ...
                    clientesFiltrados.map((cliente) => (
                        <tr key={cliente.id} className="hover:bg-slate-50 transition group cursor-pointer" onClick={() => setClienteSeleccionado(cliente)}>
                            ...
                            <td className="px-6 py-4 text-right">
                                <button className="text-blue-600 font-bold text-xs flex items-center justify-end gap-1 hover:underline">
